...
### 8. OpenAPI Schema Generation for iOS (Week 6 Day 2) - COMPLETED 2025-11-02

**Issue**: Swift API models manually maintained separately from Python Pydantic models, causing schema mismatches and runtime errors.

**Resolution**: Implemented OpenAPI code generation with Pydantic models as single source of truth.

**Implementation**:
- Downloaded OpenAPI schema from backend at `/openapi.json` (auto-generated by FastAPI from Pydantic models)
- Installed `openapi-generator` tool via Homebrew
- Generated 22 Swift Codable models in `BoggleApp/Generated/Sources/OpenAPIClient/Models/`
- Created automation script: `scripts/generate_ios_models.sh`
- Documented schema generation workflow in `claude.md`

**Schema flow**:
```
Pydantic models (src/api_models.py)
  → FastAPI generates OpenAPI schema (/openapi.json)
  → openapi-generator creates Swift models
  → iOS app imports generated models
```

**Files changed**:
- `scripts/generate_ios_models.sh` - Automation script for regenerating models
- `BoggleApp/Generated/` - 22 generated Swift model files
- `claude.md` - Added "iOS Client & Schema Generation" documentation section
- `openapi.json` - Downloaded schema (not committed, regenerated on demand)

**Benefits**:
- **No schema drift**: Generated models always match backend exactly
- **Type safety**: Compile-time errors if client/server types mismatch
- **Auto documentation**: Pydantic Field descriptions carry over to Swift
- **Consistent naming**: Generator handles snake_case → camelCase conversion
- **One command**: `./scripts/generate_ios_models.sh` regenerates all models

**Next steps** (future work):
- Add generated model files to Xcode project
- Replace manual model definitions in `BoggleAPI.swift` with generated ones
- Consider adding generation as pre-build step in Xcode
