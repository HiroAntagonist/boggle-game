...
def get_public_display_name(user: User, is_public: bool) -> str:
    """Get the display name for a user in a game context.

    For public games, ONLY returns the user's gamer_tag (never display_name or email).
    For private games, returns display_name or email prefix.

    Args:
        user: User object from database
        is_public: Whether the game is public

    Returns:
        Display name string (gamer_tag for public games, display_name for private)
    """
    if is_public:
        # For public games, NEVER return display_name or email - only gamer_tag
        return user.gamer_tag or "Anonymous"
...
async def end_game(game_id: str, db: Session) -> None:
    ...
    for gp in game_players:
        ...
        player_words[get_public_display_name(gp.user, db_game.is_public)] = words
    ...
    for gp in game_players:
        ...
        results.append(PlayerFinalResult(
            player_name=get_public_display_name(gp.user, db_game.is_public),
            words=word_results,
    ...
    if results:
        ...
        if len(winners) == 1:
            ...
            for gp in game_players:
                if get_public_display_name(gp.user, db_game.is_public) == winner:
...
async def join_game(
    ...
    # Validate that users joining public games have gamer tags
    if db_game.is_public and not current_user.gamer_tag:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="You must set a gamer tag before joining public games"
        )
    ...
    for gp in all_players:
        ...
        if user:
            player_names.append(get_public_display_name(user, db_game.is_public))
    ...
    if not is_rejoining:
        ...
        await manager.broadcast(
            json.dumps({
                "type": "player_joined",
                "player_name": get_public_display_name(current_user, db_game.is_public),
...
async def websocket_endpoint(
    ...
    try:
        ...
        if remaining_connections == 0:
            ...
            # Force fresh read from database (session may have cached stale data)
            db.expire_all()
            db_game = db.query(GameModel).filter(GameModel.id == game_id).first()
